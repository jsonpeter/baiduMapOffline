...
(function() {
    ...
    Zc.getTilesUrl = function(a, b, c) {
        ...
        return (function (C) {
            if (C.enable) {
                var url = C.host + C.path + C.tilePath+'/'+b+'/'+d+'/'+a
                    + ".png";
                return url.replace(/-(\d+)/gi, "M$1");
            } else {
                return (Xc[Math.abs(d + a) % Xc.length] + "/" + (d + "").replace(/-/gi, "M") + "/" + (a + "").replace(/-/gi, "M") + "/" + b + "&scaler=" + f + (6 == x.da.la ? "&color_dep=32&colors=50" : "") + "&udt=" + e).replace(/-(\d+)/gi, "M$1");
            }
        })(window.__BMAP_EXTRA_CONFIG__);
    };
    var Ma = new Vc("\u5730\u56fe", Zc, {
